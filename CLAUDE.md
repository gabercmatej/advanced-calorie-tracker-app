...
- Root-level `<Stack.Protected>` branches on `session` (from `AuthContext`):
  ...
  - **Signed in** → `(tabs)/` group (Home / Progress / Profile) plus modal routes: `add.tsx` (photo/description food logger, accepts an optional `?date=` param to log against a past day), `log-weight.tsx`, `entry/[id].tsx` (edit/delete a logged food), `day/[date].tsx` (full day view, reached by tapping the Home week-strip or the Progress calendar).
...
- `DiaryContext` — the main domain store: `Profile` (goals, metrics, units, theme, notification pref), `FoodEntry[]`, `WeightEntry[]`, plus derived values (`entriesForDate`, `totalsForDate`, `streak`, `loggedDates`). Screens never touch storage directly — they call `useDiary()`.
- `ThemeContext` (`ResolvedThemeProvider`) — resolves `profile.theme` ('system' | 'light' | 'dark') against the OS scheme; `useTheme()` reads through this, not directly off `useColorScheme()`.
...
**Theming.** All colors/spacing/radii live in `src/constants/theme.ts` as light/dark token objects (navy-blue brand, `onTint` for text/icons on filled tint surfaces); components consume them through `useTheme()`. Do not hardcode colors in components — add a token. `themed-text.tsx` / `themed-view.tsx` are the base primitives; `Screen`, `Card`, `Button`, `Segmented`, `Field`, `Slider`, `WeightPicker`, `DateField`, `OptionCards`, `ProgressBar`, `MacroRow`, `MacroRings`, `CalorieRing`, `WeightChart` build on them. `CalorieRing` and `WeightChart` are `react-native-svg`-based.
