 ...
 *
 * CalAI never sends a push. Every streak reminder is scheduled locally on the
 * device by `src/lib/notifications.ts`, which needs no server, no APNs token
 * and no entitlement — `aps-environment` governs remote delivery only, so
 * removing it costs the app nothing it uses.
 *
